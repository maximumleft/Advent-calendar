<template>
  <div class="max-w-md mx-auto mt-12 bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-xl shadow-2xl">
    <h2 class="text-3xl font-black mb-6 text-center text-white">Регистрация</h2>
    
    <form @submit.prevent="handleRegister" class="space-y-4">
      <div>
        <label class="block text-sm font-bold text-white/60 mb-1">Имя</label>
        <input 
          v-model="form.name" 
          type="text" 
          required 
      ...
      <div>
        <label class="block text-sm font-bold text-white/60 mb-1">Подтверждение пароля</label>
        <input 
      ...
      <div v-if="errors" class="text-red-400 text-sm bg-red-400/10 border border-red-400/20 rounded-xl p-3">
    ...
    <p class="mt-6 text-center text-white/40 text-sm">
      Уже есть аккаунт? 
      <router-link to="/login" class="text-blue-400 font-bold hover:underline">Войти</router-link>
    </p>
  </div>
</template>
